...
class GalaxyCatalogue(Catalogue):
    ...
    def add_galaxies(self, hod):
        ...
        #lm = self.haloes.get("log_mass")
        #zc = self.haloes.get("zcos")
        mag_cen = hod.get_magnitude_centrals(self.haloes.get("log_mass"), self.haloes.get("zcos"))

        num_sat = hod.get_number_satellites(self.haloes.get("log_mass"), self.haloes.get("zcos"))
        self.haloes._quantities["num_sat"]=num_sat
        
        ind_cen, mag_sat = \
            hod.get_magnitude_satellites(self.haloes.get("log_mass"), num_sat, self.haloes.get("zcos"))
    ...
    def _get_distances(self, conc="mod_conc"):
...
class GalaxyCatalogueSnapshot(GalaxyCatalogue):
    ...
    def _position_galaxies_particle(self):
        ...
        pos[pos >= self.box_size] -= self.box_size
        pos[pos < 0] += self.box_size
