...
class HOD_BGS(HOD):
    ...
    def __init__(self, mass_function=MassFunctionMXXL(), cosmology=CosmologyMXXL(), mag_faint=20.0,
        ...
        self.Mmin_Ls, self.Mmin_Mt, self.Mmin_am, self.M1_Ls, self.M1_Mt, self.M1_am, \
            ...
            self.sigma_B, self.sigma_C, self.sigma_D  = lookup.read_hod_param_file_mxxl(hod_param_file)
    ...
    def smallest_halo_mass(self, redshift, apparent_magnitude):
        """
        Returns the minimum halo mass required to host galaxies brighter than
        the specified apparent magnitude threshold
        Args:
            redshift:           array of redshift
            apparent_magnitude: array of r-band apparent magnitude threshold
        Returns:
            array of log10 mass of the minimum halo mass required to host
            galaxies brighter than the apparent magnitude threshold, where 
            halo mass is in units Msun/h.
        """
        
        # Find the faintest galaxies with corresponding apparent magnitude
        col_red = np.ones(len(redshift)) * 100
        col_blue = np.ones(len(redshift)) * -100
        abs_mag_red = self.kcorr.absolute_magnitude(apparent_magnitude, 
                                                    redshift, col_red)
        abs_mag_blue = self.kcorr.absolute_magnitude(apparent_magnitude,
                                                    redshift, col_blue)
        absolute_magnitude = np.maximum(abs_mag_red, abs_mag_blue)
        
        # array to store halo masses
        log_min_mass = np.zeros(len(redshift))
        
        # loop through magnitudes, finding smallest halo mass
        log_mass_bins = np.arange(0,16,0.01)
        for i in range(len(redshift)):
            Ncen = self.number_centrals_mean(log_mass_bins, 
                            np.ones(len(log_mass_bins))*absolute_magnitude[i], 
                                 np.ones(len(log_mass_bins))*redshift[i])
            idx = np.where(Ncen==0)[0][-1]
            log_min_mass[i] = log_mass_bins[idx]
